...
**Note**: as of July 2025, there is a new subpackage include, a custom logging facility (by no means complete, yet). It expects all struct members to include values for proper error output to logile
...
`go get "github.com/jeanfrancoisgratton/customError/v2"` (v2.x.y is the current version, have a look at the Releases section of the repo to confirm)
2. In the import section of your file, you need to add the above package in that section, something like: `import ce "github.com/jeanfrancoisgratton/customError/v2"`
3. Then, you will need to change your previous code a bit:

```go
func myFunc() error {
    if err := os.MkDirAll(var1, var2); err != nil {
        cerr := ce.CustomError {Fatality: ErrortypeIota.Fatal, Message: err.Error(), Title: "Error creating directory", Code: -8}
        return cerr.Error()		
    }
}
```
