...
func main() {
	...
	case strings.ToLower("fatal"):
		testFatal()
	case strings.ToLower("warning"):
		testWarning()
	case strings.ToLower("continuable"):
		testContinuable()
	default:
...
func testContinuable() {
	...
	ce = customError.CustomError{Fatality: customError.Continuable, Title: "WARNING", Message: "My Error message", Code: 222}
	fmt.Println(ce.Error())

	fmt.Println("Continuable with title, no code")
	ce = customError.CustomError{Fatality: customError.Continuable, Title: "WARNING", Message: "My Error message"}
	fmt.Println(ce.Error())

	fmt.Println("Continuable no title with code")
	ce = customError.CustomError{Fatality: customError.Continuable, Message: "My Error message", Code: 222}
	fmt.Println(ce.Error())
